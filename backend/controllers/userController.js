...
const writeReview = async(req,res,next)=>{
    try{
        ...
        //create review id manually because it is needed also for saving in product
        //collection
        ...
        else
        {
            ...
            product.rating = prc.map((item)=>Number(item.rating)).reduce((sum,item)=>
            sum+item,0)/product.reviews.length
        }
        await product.save()
